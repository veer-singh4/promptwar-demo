...
export default function Location() {
  ...
  return (
    <div className={cn(
      "flex flex-col space-y-6 animate-in fade-in duration-500 pb-16",
      ...
      <div 
        className={cn(
          "w-full overflow-hidden shadow-2xl relative transition-all duration-500",
          isExpanded 
            ? "fixed inset-0 z-[300] bg-black" 
            : "h-[360px] rounded-3xl border border-[var(--color-navy-border)]"
        )}
      >
        <Map
          ...
          mapId="attendee_tracking_map_v3"
        >
          <AdvancedMarker position={stadiumLocation}>
             <Pin background={'#ef4444'} glyphColor={'#fff'} borderColor={'#991b1b'} scale={isExpanded ? 1.5 : 1.2} />
          </AdvancedMarker>
        </Map>

        {/* Map Overlays */}
        <div className="absolute top-4 right-4 flex flex-col gap-2 z-[310]">
          <button 
            onClick={() => setIsExpanded(!isExpanded)}
            className="w-12 h-12 bg-white/90 backdrop-blur-md rounded-2xl flex items-center justify-center text-[var(--color-navy-base)] shadow-2xl active:scale-90 transition-all border border-black/10"
          >
        ...
        {isExpanded && routeInfo && (
          <div className="absolute bottom-10 left-4 right-4 z-[310] animate-in slide-in-from-bottom duration-500">
             <div className="bg-[var(--color-navy-card)]/90 backdrop-blur-xl border border-white/10 rounded-[2rem] p-6 shadow-2xl flex justify-between items-center text-white">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-[#3b82f6] mb-1">Estimated Arrival</p>
                  <h2 className="text-3xl font-black">{routeInfo.duration.text}</h2>
                  <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase">{routeInfo.distance.text} • {travelMode}</p>
                </div>
                <button 
                  onClick={() => setIsExpanded(false)}
                  className="w-12 h-12 bg-red-500/20 text-red-400 rounded-full flex items-center justify-center border border-red-500/30"
                >
                  <X size={20} />
                </button>
             </div>
            ...
            <section className="space-y-4 pb-12">
              ...
              <div className="bg-[var(--color-navy-card)] border border-[var(--color-navy-border)] rounded-3xl overflow-hidden shadow-xl max-h-[400px] overflow-y-auto custom-scrollbar">
                {routeInfo.steps.map((step, idx) => (
                  <div key={idx} className="p-5 flex gap-5 items-start border-b border-white/5 last:border-0 hover:bg-white/[0.02]">
                    <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center shrink-0 border border-slate-700">
                      <span className="text-[10px] font-black text-[var(--color-accent-blue)]">{idx + 1}</span>
                    </div>
                    <div className="flex-1 space-y-1">
                      <p className="text-white text-xs font-semibold leading-relaxed" dangerouslySetInnerHTML={{ __html: step.instructions }} />
